...
namespace Repositorios.Sso
{
    public class UserProviderRepositorio : IUserProvider
    {
        ...
        public string ObtenerCorreoUsuarioSso(SSOObtenerCorreo SSOReq)
        {
            ...
            {
                var serviceEndpointUri = new Uri(_configuracion.ServiceEndpoint);
                var usuarioCliente = new UsuarioClient(serviceEndpointUri, am);
                return usuarioCliente.Obtener(SSOReq.Nit).Correo;
            }
        }
        ...
        public UsuarioSsoModelo ObtenerUsuarioNit(SSOObtenerUsuarioNit SSOReq)
        {
            using (var am = new AuthenticationManager(SSOReq.PrivateKeyXml))
            {
                ...
                try
                {
                    Usuario usuario = usuarioCliente.Obtener(SSOReq.Nit);

                    if(usuario != null)
                    {
                        var usuarioSicorvip = new UsuarioSsoModelo()
                        {
                            Nit = usuario.Nit,
                            Nombre = usuario.Nombre,
                            Correo = usuario.Correo,
